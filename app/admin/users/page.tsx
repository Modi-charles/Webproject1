...
import { LayoutDashboard, Users, ImageIcon, Film, Tv, Star, Sparkles, Music, Wallet, LogOut, Home, Crown, Menu, X, ChevronRight } from 'lucide-react'
    ...
    <div className="min-h-screen bg-background pb-16 md:pb-0">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border safe-area-bottom">
        <div className="grid grid-cols-5 gap-0">
          ...
          >
            ...
            <span className={`text-[9px] ${mobileMenuOpen || navItems.slice(4).some((item) => item.view === activeView) ? 'font-semibold' : ''}`}>More</span>
          </button>
        </div>
      </nav>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-black/50" onClick={() => setMobileMenuOpen(false)}>
          <div
            className="absolute bottom-16 left-0 right-0 bg-card border-t border-border rounded-t-2xl p-3 safe-area-bottom"
